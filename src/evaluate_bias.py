...
# 0. Config
# -----------------------
MODEL_PATH = "models/bias_roberta3"
...
# -----------------------
# 1. Load Dataset Same Way as Training
# -----------------------
...
metrics_path = os.path.join(RESULTS_DIR, "bias_metrics.json")
...
results_df["left_probability"] = probs[:, 0]
results_df["neutral_probability"] = probs[:, 1]
results_df["right_probability"] = probs[:, 2]

results_df["confidence"] = confidence
results_df["correct"] = true_labels == pred_labels

predictions_path = os.path.join(RESULTS_DIR, "bias_predictions.csv")
...
errors_path = os.path.join(RESULTS_DIR, "bias_errors.csv")
...
plt.title("Bias Model Confusion Matrix")
plt.tight_layout()

cm_path = os.path.join(RESULTS_DIR, "bias_confusion_matrix.png")
...
# -----------------------
# 9. Error Type Summary
# -----------------------
error_summary = errors_df.groupby(
    ["true_label", "predicted_label"]
).size().reset_index(name="count").sort_values("count", ascending=False)

error_summary_path = os.path.join(RESULTS_DIR, "bias_error_summary.csv")
...
confidence_path = os.path.join(RESULTS_DIR, "bias_confidence_bins.csv")
...
plt.ylabel("Accuracy")
plt.title("Bias Accuracy by Confidence Range")
plt.tight_layout()

confidence_plot_path = os.path.join(RESULTS_DIR, "bias_confidence_bins.png")
